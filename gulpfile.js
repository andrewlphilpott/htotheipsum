...
// Minify JS Libraries, Modules, and Controllers
gulp.task('app', function() {
    return gulp.src(paths.src.jsAll)
        .pipe(uglify())
        .on('error', function(err){
            notify().write(err);
            this.emit('end');
        ...
        .pipe(gulp.dest(paths.dest.js))
        .pipe(notify({message: 'App minified'}));;
});
...
gulp.task('watch', function() {
    ...
    gulp.watch(paths.src.jsAll, ['app']);
...
// Default Task
gulp.task('default', ['imagemin', 'app', 'plugins', 'sass', 'scripts', 'serve', 'twig', 'watch']);
